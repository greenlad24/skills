...
def save(values: dict[str, str]) -> dict[str, list[str]]:
    ...
    A blank value is treated as "leave unchanged" so submitting the wizard with an
    already-configured field left empty never wipes the saved secret.
    """
    ...
    updates = {k: v for k, v in cleaned.items() if v != ""}
    skipped_blank = [k for k, v in cleaned.items() if v == ""]
    ignored = [k for k in values if k not in ALLOWED_KEYS]
    if updates:
        upsert_env(updates)
        apply_live(updates)
    return {"saved": sorted(updates), "ignored": sorted(ignored), "skipped_blank": sorted(skipped_blank)}
